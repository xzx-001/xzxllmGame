...
export class BridgeTemplates {
    ...
    getBridge(success, mood, timeSpent) {
        const condition = success ? 'success' : 'failure';
        const candidates = this.templates.filter(t => t.condition === condition && t.moods.includes(mood));
        ...
        if (timeSpent !== undefined && candidates.some(c => c.timeAware)) {
            if (timeSpent > 300 && !success) {
                const encouraging = candidates.find(c => c.moods.includes(AIMood.CONCERNED));
                if (encouraging) {
...
//# sourceMappingURL=bridge-templates.js.map